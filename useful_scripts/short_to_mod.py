#!/usr/bin/python
# input assumes the short points are represented pointwise.
# output radii are outer bin radii and fluid values are cell-averaged
import sys
...
output_length = (len(r_f)-1) * interpolation_multiplier + 1
if(r_f[0]==0):
    output_length = output_length-1

r   = np.ones(output_length)
T   = np.ones(output_length)
rho = np.ones(output_length)
Ye  = np.ones(output_length)
print len(r)
