...
plot pi*4.*pi*r*r*x*x*x*h/c/c*1/(exp(h*x/(k_b*T))+1.)/6., './species0_I1.spec' using 1:2
set output

set term pdf
set output "compare_spectrum_1.pdf"
set title "Electron Anti-Neutrinos"
plot 4.*pi*pi*r*r*x*x*x*h/c/c*1/(exp(h*x/(k_b*T))+1.)/6., './species1_I1.spec' using 1:2
set output

set term pdf
set output "compare_spectrum_2.pdf"
set title "Mu/Tau Anti/Neutrinos"
plot 4.*pi*pi*r*r*x*x*x*h/c/c*1/(exp(h*x/(k_b*T))+1.)*2./3., './species2_I1.spec' using 1:2
set output
